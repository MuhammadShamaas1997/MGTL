...
class material_dielectric():#{{{
    ...
    """
    def __init__(self, where=None, eps=2.):
        self.eps = eps
        self.pol = [
                ]
...
class material_DrudeMetal():#{{{
    ...
    Losses are introduced by letting their velocity be randomized with the scattering 
    frequency gamma.
    ...
    def __init__(self, where=None, lfconductivity=15e6, f_c=1e14):
        """
...
class material_STO_THz():#{{{  ## for THz application only
    ...
    def __init__(self, where=None):
        self.eps = 100.
        self.pol = [
                {'omega':2e12, 'gamma':.5e12, 'sigma':210.},
                ]
...
class material_STO():#{{{  ## for THz application only
    """
    STO
