...
double dxmin=0.0, dxmax=0.5, dymin=0.0, dymax=5, dzmin=0.0, dzmax=0.5;
...
{
  cdouble Vy=line_integral_y(f,Hy,0.0001,ycen-dymax,y,xcen,zcen);
...
{
  cdouble Vy=line_integral_y(f,Ey,0.0001,ycen-dymax,y,xcen,zcen);
...
int main(int argc, char *argv[]) {
  ...
  const char *mydirname = "MMTL-out";
    ...
    for (theta = 0; theta <= (2*pi); theta=theta+0.1)
    {
    	xpcoord[numcoord]=rp*cos(theta);
    	ypcoord[numcoord]= -5.0;
    	zpcoord[numcoord]=rp*sin(theta);
    	xscoord[numcoord]=rp*cos(theta);
    	yscoord[numcoord]=5.0;
    	zscoord[numcoord]=rp*sin(theta);
    	numcoord++;
    }

    grid_volume gv = vol3d(5.0, 15.0, 5.0, divisions);
    ...
    */my_medium_struct.H_chi2_diag.x=1.0;
    ...
    my_medium_struct.H_susceptibilities.items[0].frequency = NiFe_frq;
  ...
  geometric_object_list g = {1, objects};
    ...
    */      
    ...
    for (double fp=0.012;fp>=0;fp=fp-0.0001)
    {
    	for (double yp = -7.5; yp <= 7.5; yp=yp+1)
    	{
    		cout<<f.get_mu(vec(0.0,yp,0.0),fp)-1000.0<<" ";
    	}
    	cout<<endl;
    }
    ...
    for (int i=0;i<numcoord;i++)
    {
      ...
      for (double fr = 0.01; fr <= 0.01; fr=fr+0.01)
      {
        continuous_src_time src(cdouble(fr,0.0));
        theta=acos(xpcoord[i]/rp);
        f.add_volume_source(Ez, src, vsrc1, cdouble(amplitude*cos(theta),0));
        f.add_volume_source(Ex, src, vsrc1, cdouble(amplitude*sin(theta),0));
      }
    ...
    ycenp=ycen-dymin-(0.5*wcore); 
    zcenp=zcen;
    volume box1( vec(dxmax,-5.0,dzmax), vec(-dxmax,-4.8,-dzmax) );
    ycenp=ycen+dymin+(0.5*wcore); 
    volume box2( vec(dxmax,4.8,dzmax), vec(-dxmax,5.0,-dzmax) );

    fcen = (1e9)/f0; // ; pulse center frequency
    df = 0.001*(fcen/f0);    // ; df
  
    double fmin = 0, fmax = 0.2;
    int Nfreq = 1000;
  ...
  */
  ...
  f.step();
  f.step();
    volume vxy=volume(vec(-15,-15,0),vec(15,15,0));
    volume vxz=volume(vec(-15,0,-15),vec(15,0,15));
    volume vyz=volume(vec(0,-15,-15),vec(0,15,15));
    ...
    f.output_hdf5(Hz,vxz);
    f.output_hdf5(Bx,vxz);
    f.output_hdf5(By,vxz);
    f.output_hdf5(Bz,vxz);
    f.output_hdf5(Ex,vxz);
    f.output_hdf5(Ey,vxz);
    f.output_hdf5(Ez,vxz);
    f.output_hdf5(Dx,vxz);
    f.output_hdf5(Dy,vxz);
    f.output_hdf5(Dz,vxz);
    f.output_hdf5(Sx,vxz);
    f.output_hdf5(Sy,vxz);
    f.output_hdf5(Sz,vxz);
    ...
    f.get_eigenmode_coefficients(flux1,box1,bands,num_bands,3,divisions,DEFAULT_SUBPIXEL_TOL,coeffs,vgrp);
	...
	*/
    ...
    for (double z=(dzmax);z>=(-dzmax);z=z-0.01)  
    {
    	monitor_point pin;
