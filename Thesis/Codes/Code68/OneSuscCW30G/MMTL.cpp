...
{
  cdouble Iyf=line_integral_y(f,Ey,0.01,ycen-dymax,ycen+dymax,xcen+dxmax,z);
    cdouble Iyb=line_integral_y(f,Ey,0.01,ycen-dymax,ycen+dymax,xcen-dxmax,z);
    cdouble Ixt=line_integral_x(f,Ex,0.01,xcen-dxmax,xcen+dxmax,ycen-dymax,z);
    cdouble Ixb=line_integral_x(f,Ex,0.01,xcen-dxmax,xcen+dxmax,ycen+dymax,z);
...
{
  cdouble Iyf=line_integral_y(f,Hy,0.01,ycen-dymax,ycen+dymax,xcen+dxmax,z);
    cdouble Iyb=line_integral_y(f,Hy,0.01,ycen-dymax,ycen+dymax,xcen-dxmax,z);
    cdouble Ixt=line_integral_x(f,Hx,0.01,xcen-dxmax,xcen+dxmax,ycen-dymax,z);
    cdouble Ixb=line_integral_x(f,Hx,0.01,xcen-dxmax,xcen+dxmax,ycen+dymax,z);
...
cdouble compute_Vm(fields & f, double z)
{
  ...
  cdouble Vz=line_integral_y(f,Hy,0.01,-25.0,0.0,xcen,z);
  //cdouble Vm=Vy+Vz;
  return Vz;
}
...
cdouble compute_Ve(fields & f, double z)
{
  ...
  cdouble Vz=line_integral_y(f,Ey,0.01,-25.0,0.0,xcen,z);
  //cdouble Ve=Vy+Vz;
  return Vz;
}
...
{ 
  ...
  double sigp=0.0;
  if(in_middle){sigp=0.000001;}
  else{sigp=0.0;}
  return sigp;
}

double fchi2(const vec &v) 
{ 
    bool in_middle=false;
...
/*    m->H_susceptibilities.num_items = 1;
  ...
  double dxp=p.x - xpcoord[i];
    ...
    double drp=sqrt(dxp*dxp+dyp*dyp+dzp*dzp);
    if((dxp<=(winding_thickness_p/2.0))&&(dyp<=(winding_thickness_p/2.0))&&(dzp<=(winding_thickness_p/2.0))){
    ...
    if((dxs<=(winding_thickness_p/2.0))&&(dys<=(winding_thickness_p/2.0))&&(dzs<=(winding_thickness_p/2.0))){
      //m->D_conductivity_diag.x = m->D_conductivity_diag.y = m->D_conductivity_diag.z = (3.5e7)/sigmaD0;
    }
  }
...
int main(int argc, char *argv[]) {
  ...
  //double y=0.0;
  {for (double z=-dzmax;z<=30;z=z+00.1){
...
/*  for(double z=-dzmax;z<=dzmax;z=z+(2*dzmax))
  ...
  {for (double y=-12.5;y<=12.5;y=y+00.1){
    ...
    structure transformer = new structure(gv, eps,pml(3.0));
  ...
  vector3 size2 = {wcore, wcore, 45.0};
  ...
  geometric_object_list g = {1, objects};
    ...
    //transformer.add_susceptibility(sigma2, H_stuff, gyrotropic_susceptibility(vec(0.0,0.0,b0),(1.0), (33e-2)/(2*3.1459),1.0,GYROTROPIC_SATURATED));
    transformer.add_susceptibility(sigma, H_stuff, lorentzian_susceptibility(5.8e-5, (0.33e-2)/(2*3.1459)));

    fields f(& transformer);
    f.use_real_fields();
    //(& transformer)->set_chi2(fchi2);
    //(& transformer)->set_chi3(fchi3);	
...
for (double fp=0.0;fp<=(1e9)/f0;fp=fp+(1e4)/f0)
    ...
    //f_range;1e-5,1e-2
    double fcen = (3e10)/f0; // ; pulse center frequency
...
int stop=0;
    ...
    for (int i = 0; i < Nfreq; ++i) {
      ...
      //freq , fluxin , fluxout
       ...
       if (i<=2000)
      {
        //cdouble Vmi=compute_Vm(f,-10.0);
        //cdouble Imi=compute_Im(f,-10.0);
        //cdouble Vmo=compute_Vm(f, 10.0);
        //cdouble Imo=compute_Im(f, 10.0);
        //cdouble Ve=compute_Ve(f,zcen);
        //cdouble Ie=compute_Ie(f,zcen);
        //Time<<Imi.real()<<" , "<<Imi.imag()<<" , "<<Vmi.real()<<" , "<<Vmi.imag()<<Imo.real()<<" , "<<Imo.imag()<<" , "<<Vmo.real()<<" , "<<Vmo.imag()<<endl;//" , "<<Ie.real()<<" , "<<Ie.imag()<<" , "<<Ve.real()<<" , "<<Ve.imag()<<endl;
        //Im , Vm , Ie , Ve 
