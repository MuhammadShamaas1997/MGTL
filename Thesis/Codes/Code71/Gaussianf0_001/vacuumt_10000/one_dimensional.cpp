...
int main(int argc, char **argv) {
  ...
  //s.add_susceptibility(two, H_stuff, gyrotropic_susceptibility(vec(0.0,0.0,1.0),0.001, 0.001,0.00001,GYROTROPIC_SATURATED));
  //bias omega gamma alpha type

  fields f(&s);
  //f.use_real_fields();
  //continuous_src_time src(1.0);
  gaussian_src_time src(0.00075,0.0015);

for (double fp=0.0;fp<=0.002;fp=fp+0.000001)
    { 
      double yp = 0.0;
      {
  ...
  f.add_point_source(Hz,src,vec(0.0,0.0,-4.5));
    ...
    volume vxz=volume(vec(-6,0,-6),vec(6,0,6));
  int tind=0;
  while (f.time() < ttot) {
	f.step();tind++;
