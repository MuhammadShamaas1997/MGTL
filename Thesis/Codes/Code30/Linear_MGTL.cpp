...
int main(int argc, char *argv[]) {
  ...
  const char *mydirname = "MMTL-out";
    ...
    Susceptibility.open ("Susceptibility.txt");
  ...
  vector3 size = {2.0, 15.0, 2.0};
  //objects[0] = make_block(my_material, center, radius, height, zhat);
  objects[0] = make_block(my_user_material, center, xhat, yhat, zhat, size);
  geometric_object_list g = {1, objects};
    ...
    double maxf=1000;
    long n=0;
    for (double f=0; f<=10; f=f+0.000001){
        n=n+1;
        if(n%1000000==0){cout<<f<<endl;}
        {Susceptibility<<f<<" "<<transformer.get_mu(vec(0.0,0.0,0.0),f)<<endl;}
    }
