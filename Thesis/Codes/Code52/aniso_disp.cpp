...
public:
  virtual double mu(const vec &r) { return 1000; }
...
int main(int argc, char **argv) {
  ...
  if (0 == divide_parallel_processes(count_processors())) {
    ...
    s->add_susceptibility(anisodispmat, H_stuff, lorentzian_susceptibility(1.1, 1));
    
    for (double f=-100; f<=100; f=f+1){
      //if(transformer.get_mu(vec(0.1,6.9,f),1)!=1000)
        {cout<<s->get_mu(vec(0,0,1),f)<<endl;}
    }
