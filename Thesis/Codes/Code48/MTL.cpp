...
void my_material_func(vector3 p, void *user_data, meep_geom::medium_struct *m) {
  ...
  if (in_middle)
  {
    m->E_susceptibilities.num_items = 1;
    m->E_susceptibilities.items = new meep_geom::susceptibility[1];
    ...
    m->E_susceptibilities.items[0].sigma_diag.x = 0.5;//NiFe_sig;
    m->E_susceptibilities.items[0].sigma_diag.y = 0.5;//NiFe_sig;
    m->E_susceptibilities.items[0].sigma_diag.z = 0.5;//NiFe_sig;
    m->E_susceptibilities.items[0].bias.x = 0.0;
    m->E_susceptibilities.items[0].bias.y = 0.0;
    m->E_susceptibilities.items[0].bias.z = 0.0;
    m->E_susceptibilities.items[0].frequency = (1e9)/f0;//NiFe_frq;
    m->E_susceptibilities.items[0].gamma = 1e-6;//*((10e9)/f0);//NiFe_gam;
    m->E_susceptibilities.items[0].alpha = 0.0;//gaussian_random(1.0,1.0);//NiFe_alpha;
    m->E_susceptibilities.items[0].noise_amp = 0.0;
    m->E_susceptibilities.items[0].drude = false;
    m->E_susceptibilities.items[0].saturated_gyrotropy = true;
...
class anisodisp_materialE : public material_function {
public:
  virtual void sigma_row(component c, double sigrow[3], const vec &r) {
    (void)r; // unused
    if (component_direction(c) == X) {
...
int main(int argc, char *argv[]) {
  ...
  const char *mydirname = "MMTL-out";
    ...
    for (theta = 0.0; theta <= (2*pi); theta=theta+0.1)
    {
      ...
      zscoord[numcoord]=z+20.0;
      numcoord++;
    }

  double y=0.0;
  {for (double z=-10.0;z<=10.0;z=z+00.1){
  ...
  {for (double y=-12.5;y<=12.5;y=y+00.1){
    ...
    //transformer.add_susceptibility(anisodispmat, E_stuff, lorentzian_susceptibility(1.1, 1e-5));
    //transformer.add_susceptibility(anisodispmat, H_stuff, lorentzian_susceptibility(1.1, 1e-5));
    //fields f(& transformer);

    /*for (double fp=0.0;fp<=2.0;fp=fp+0.01)
    { 
      double yp = 0.0;
      {
        Permeability<<(fp*f0)<<" "<<f.get_mu(vec(0.1,0.1,0.1),fp)-2<<endl;
        Permittivity<<(fp*f0)<<" "<<f.get_eps(vec(0.1,0.1,0.1),fp)-10<<endl;
    ...
    */my_medium_struct.H_chi2_diag.x=1e-3;
    my_medium_struct.H_chi2_diag.y=1e-3;
    my_medium_struct.H_chi2_diag.z=1e-3;
    my_medium_struct.H_chi3_diag.x=1e-3;
    my_medium_struct.H_chi3_diag.y=1e-3;
    my_medium_struct.H_chi3_diag.z=1e-3;
    ...
    my_medium_struct.E_susceptibilities.num_items = 1;
    my_medium_struct.E_susceptibilities.items = new meep_geom::susceptibility[1];

    my_medium_struct.E_susceptibilities.items[0].sigma_offdiag.x = 0.0;
    my_medium_struct.E_susceptibilities.items[0].sigma_offdiag.y = 0.0;
    my_medium_struct.E_susceptibilities.items[0].sigma_offdiag.z = 0.0;
    my_medium_struct.E_susceptibilities.items[0].sigma_diag.x = 0.5;
    my_medium_struct.E_susceptibilities.items[0].sigma_diag.y = 0.5;
    my_medium_struct.E_susceptibilities.items[0].sigma_diag.z = 0.5;
    my_medium_struct.E_susceptibilities.items[0].bias.x = 0.0;
    my_medium_struct.E_susceptibilities.items[0].bias.y = 0.0;
    my_medium_struct.E_susceptibilities.items[0].bias.z = 1.0;
    my_medium_struct.E_susceptibilities.items[0].frequency = ((1e9)/f0);
    my_medium_struct.E_susceptibilities.items[0].gamma = 1e-6;//10.0*((10e9)/f0);
    my_medium_struct.E_susceptibilities.items[0].alpha = 0.0;//gaussian_random(1.0,1.0);
    my_medium_struct.E_susceptibilities.items[0].noise_amp = 0.0;
    my_medium_struct.E_susceptibilities.items[0].drude = true;
    my_medium_struct.E_susceptibilities.items[0].saturated_gyrotropy = false;
    my_medium_struct.E_susceptibilities.items[0].is_file = false;

/*    my_medium_struct.E_susceptibilities.items[1].sigma_offdiag.x = 0.0;
    my_medium_struct.E_susceptibilities.items[1].sigma_offdiag.y = 0.0;
    my_medium_struct.E_susceptibilities.items[1].sigma_offdiag.z = 0.0;
    my_medium_struct.E_susceptibilities.items[1].sigma_diag.x = 0.5;
    my_medium_struct.E_susceptibilities.items[1].sigma_diag.y = 0.5;
    my_medium_struct.E_susceptibilities.items[1].sigma_diag.z = 0.5;
    my_medium_struct.E_susceptibilities.items[1].bias.x = 0.0;
    my_medium_struct.E_susceptibilities.items[1].bias.y = 0.0;
    my_medium_struct.E_susceptibilities.items[1].bias.z = 1.0;
    my_medium_struct.E_susceptibilities.items[1].frequency = 0.5;
    my_medium_struct.E_susceptibilities.items[1].gamma = 1e-5;
    my_medium_struct.E_susceptibilities.items[1].alpha = 0.0;
    my_medium_struct.E_susceptibilities.items[1].noise_amp = 0.0;
    my_medium_struct.E_susceptibilities.items[1].drude = true;
    my_medium_struct.E_susceptibilities.items[1].saturated_gyrotropy = false;
    my_medium_struct.E_susceptibilities.items[1].is_file = false;
*/
  ...
  //meep_geom::material_type default_material =meep_geom::make_user_material(my_material_func, (void *)&data, false);
  //default_material->which_subclass = meepgeom::MEDIUM_USER;
  ...
  //vector3 center1 = {0.0, -12.5, 0.0};
  vector3 center2 = {0.0, 0.0, 0.0};
  //vector3 center3 = {0.0, 12.5, 0.0};
  //vector3 center4 = {0.0, 0.0, 07.5};
  //vector3 center5 = {0.0, 0.0, -07.5};
  double radius = 3.0;
  double height = 1.0e20;
  vector3 xhat1 = {1.0, 0.0, 0.0};
  vector3 yhat1 = {0.0, 1.0, 0.0};
  vector3 zhat1 = {0.0, 0.0, 1.0};
  //vector3 size1 = {wcore, wcore, 20.0};
  vector3 size2 = {wcore, wcore, 20.0};
  //vector3 size3 = {wcore, wcore, 20.0};
  //vector3 size4 = {wcore, 30.0, wcore};
  //vector3 size5 = {wcore, 30.0, wcore};
  ...
  geometric_object_list g = {1, objects};
    ...
    */      
    ...
    anisodisp_materialE anisodispmatE;
    transformer.add_susceptibility(anisodispmatE, E_stuff, lorentzian_susceptibility((1e9)/f0, 1e-6));
...
for (double fp=0.0;fp<=(10e9)/f0;fp=fp+(0.1e9)/f0)
    { 
      double yp = 0.0;
      {
    ...
    //f_range;1e-5,1e-2
    double fcen = (10e9)/f0; // ; pulse center frequency
    double df = 0.999999*((10e9)/f0);    // ; df
...
for (int i=0;i<numcoord;i++)
    ...
    zcenp=zcen;
    volume box1( vec(dxmax,-dymax,-10.0), vec(-dxmax,dymax,0.0) );
    ycenp=ycen+dymin+(0.5*wcore); 
    volume box2( vec(dxmax,-dymax,0.0), vec(-dxmax,dymax,10.0) );

    fcen = (5e9)/f0; // ; pulse center frequency
    df = 0.001*(fcen/f0);    // ; df
  
    double fmin = 0, fmax = (1e10)/f0;
    int Nfreq = 1000;
...
int stop=0;
    ...
    f.output_hdf5(Sz,vxy);
       ...
       //if ((i%500)==0)
      {
        ...
        /*monitor_point pin;
        f.get_point(&pin, vec(xcen,ycen,-7.5));
        ...
        monitor_point po;
        f.get_point(&po, vec(xcen,ycen,07.5));
        ...
        */
        for (double z=-10.0;z<=10.0;z=z+0.25)  
      {
        monitor_point pin;
    ...
    //f.get_eigenmode_coefficients(flux1,vg,bands,num_bands,5,divisions,DEFAULT_SUBPIXEL_TOL,coeffs,vgrp);
  ...
  */
    ...
    for (double z=-10.0;z<=10.0;z=z+1.0)  
    {
