...
muinf=5;
n=1;
for omega=0:10e5:100e9
    muM(n)=(1+(i*sigmab/omega))*(muinf+(sigma0*omega0M)/(omega0M*omega0M-omega*omega-i*omega*gamma0));
    muF(n)=mu0+mudc/(1+(omega/omega0)*(omega/omega0))-1i*mudc/(1+(omega/omega0)*(omega/omega0))*(omega/omega0);
    n=n+1;
end
hold on;
%plot(0:10e5:100e9,real(muF)/mu0)
plot(0:10e5:100e9,imag(muF)/mu0,'r')
%plot(0:10e5:100e9,real(muM),'o')
plot(0:10e5:100e9,imag(muM),'*')
%axis([0 100e9 -10 10])
