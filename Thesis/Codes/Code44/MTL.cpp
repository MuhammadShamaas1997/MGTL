...
double dxmin=0.0, dxmax=0.5, dymin=4.0, dymax=5.0, dzmin=4.0, dzmax=5.0;
...
int Np=20;//must be odd
int Ns=2;//must be odd
double margin=0.2;   
...
double theta=0.0;
double rp=(wcore/2.0)+(winding_thickness_p)+insulation_thickness_p;
...
{
  cdouble Izf=line_integral_z(f,Ez,0.001,zcen+dzmin,zcen+dzmax,xcen+dxmax,y);
    cdouble Izb=line_integral_z(f,Ez,0.001,zcen+dzmin,zcen+dzmax,xcen-dxmax,y);
    cdouble Ixt=line_integral_x(f,Ex,0.001,xcen-dxmax,xcen+dxmax,y,zcen+dzmax);
    cdouble Ixb=line_integral_x(f,Ex,0.001,xcen-dxmax,xcen+dxmax,y,zcen+dzmin);
...
{
  cdouble Izf=line_integral_z(f,Hz,0.001,zcen+dzmin,zcen+dzmax,xcen+dxmax,y);
  cdouble Izb=line_integral_z(f,Hz,0.001,zcen+dzmin,zcen+dzmax,xcen-dxmax,y);
  cdouble Ixt=line_integral_x(f,Hx,0.001,xcen-dxmax,xcen+dxmax,y,zcen+dzmax);
  cdouble Ixb=line_integral_x(f,Hx,0.001,xcen-dxmax,xcen+dxmax,y,zcen+dzmin);
...
cdouble compute_Vm(fields & f, double y)
{
  ...
  cdouble Vz=line_integral_z(f,Hz,0.001,zcen-dzmin,zcen+dzmin,xcen,y);
  cdouble Vm=Vy+Vz;
  return Vm;
}
...
cdouble compute_Ve(fields & f, double y)
{
  ...
  cdouble Vz=line_integral_z(f,Ez,0.001,zcen-dzmin,zcen+dzmin,xcen,y);
  cdouble Ve=Vy+Vz;
  return Ve;
}
...
void my_material_func(vector3 p, void *user_data, meep_geom::medium_struct *m) {
  ...
  if (in_middle)
  {
    ...
    m->H_susceptibilities.items[0].sigma_diag.x = 1.0;//NiFe_sig;
    m->H_susceptibilities.items[0].sigma_diag.y = 1.0;//NiFe_sig;
    m->H_susceptibilities.items[0].sigma_diag.z = 1.0;//NiFe_sig;
    m->H_susceptibilities.items[0].bias.x = 0.0;
    m->H_susceptibilities.items[0].bias.y = 0.0;
    m->H_susceptibilities.items[0].bias.z = 0.0;
    m->H_susceptibilities.items[0].frequency = (1e9)/f0;//NiFe_frq;
    m->H_susceptibilities.items[0].gamma = (100e9)/f0;//NiFe_gam;
    m->H_susceptibilities.items[0].alpha = 1.0;//NiFe_gam;
...
int main(int argc, char *argv[]) {
  ...
  const char *mydirname = "MMTL-out";
    ...
    for (double z = -dzmin; z <= dzmin; z=z+0.1)
    {
    	theta=k*(z+dzmin);
    ...
    */my_medium_struct.H_chi2_diag.x=1.0;
    ...
    my_medium_struct.H_susceptibilities.items[0].sigma_diag.x = 1.0;
    my_medium_struct.H_susceptibilities.items[0].sigma_diag.y = 1.0;
    my_medium_struct.H_susceptibilities.items[0].sigma_diag.z = 1.0;
    ...
    my_medium_struct.H_susceptibilities.items[0].gamma = 100.0*(1e9/f0);
    my_medium_struct.H_susceptibilities.items[0].alpha = 1.0;
 ...
 for (double fp=0.0;fp<=3.0*(1e9/f0);fp=fp+0.5*(1e9/f0))
    { cout<<fp<<endl;
      for (double yp = -dymax-2.0; yp <= dymax+2.0; yp=yp+1.0)
      {
    ...
    ycenp=ycen-dymin-(0.5*wcore); 
    zcenp=zcen;
    volume box1( vec(dxmax,-dymax,dzmin), vec(-dxmax,-dymin,-dzmin) );
    ycenp=ycen+dymin+(0.5*wcore); 
    volume box2( vec(dxmax,dymin,dzmin), vec(-dxmax,dymax,-dzmin) );

    fcen = (1e9)/f0; // ; pulse center frequency
    df = 0.001*(fcen/f0);    // ; df
...
int stop=0;
	...
	*/
    ...
    for (double z=dzmax;z>=dzmin;z=z-0.001)  
    {
    	monitor_point pin;
    ...
    for (double y=(ycen-dymin);y<=(ycen+dymin);y=y+0.1)  
    {
