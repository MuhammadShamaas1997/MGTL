...
int main(int argc, char *argv[]) {
  ...
  const char *mydirname = "MMTL-out";
    ...
    for (theta = 0; theta <= (2*pi); theta=theta+0.001)
    {
    ...
    grid_volume gv = vol3d(15.0, 15.0, 15.0, divisions);
  ...
  geometric_object_list g = {1, objects};
    ...
    */      
    ...
    for (int i=0;i<numcoord;i++)
    {
      ...
      for (double fr = 0.01; fr <= 0.1; fr=fr+0.01)
      {
  ...
  */
  ...
  f.step();
  f.step();
    volume vxy=volume(vec(-7.5,-7.5,0),vec(7.5,7.5,0));
    volume vxz=volume(vec(-7.5,0,-7.5),vec(7.5,0,7.5));
    volume vyz=volume(vec(0,-7.5,-7.5),vec(0,7.5,7.5));
    ...
    f.get_eigenmode_coefficients(flux1,box1,bands,num_bands,1,ODD_Y,DEFAULT_SUBPIXEL_TOL,DEFAULT_SUBPIXEL_TOL,coeffs,vgrp);
