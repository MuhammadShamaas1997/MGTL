...
L2=32768*4;
Hx=[zeros(L2-Lmax,121); Hx(1:Lmax,:)]; 
Hy=[zeros(L2-Lmax,121); Hy(1:Lmax,:)]; 
Hz=[zeros(L2-Lmax,121); Hz(1:Lmax,:)]; 
Ex=[zeros(L2-Lmax,121); Ex(1:Lmax,:)]; 
Ey=[zeros(L2-Lmax,121); Ey(1:Lmax,:)]; 
Ez=[zeros(L2-Lmax,121); Ez(1:Lmax,:)]; 
...
% figure;surf(abs(imag(Sz(1:200,:))));hold on;
 figure;surf((1:121).*(a0/4),(1:Lmax).*(t0/4),Sz(1:Lmax,1:121).*E0.*H0);hold on;
