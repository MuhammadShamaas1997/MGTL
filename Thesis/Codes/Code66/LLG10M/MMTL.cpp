...
/*  for(double z=-dzmax;z<=dzmax;z=z+(2*dzmax))
  ...
  geometric_object_list g = {1, objects};
    ...
    transformer.add_susceptibility(sigma, H_stuff, gyrotropic_susceptibility(vec(0.0,0.0,b0),(5.8e-5), (-0.33e-2)/(2*3.1459),1e6,GYROTROPIC_SATURATED));
...
for (double fp=0.0;fp<=(1e9)/f0;fp=fp+(1e4)/f0)
    ...
    //f_range;1e-5,1e-2
    double fcen = (.005e9)/f0; // ; pulse center frequency
    double df = 0.999999*((0.01e9)/f0);    // ; df
    //continuous_src_time src(cdouble(fcen,df));
    gaussian_src_time src(fcen,df);

for (double fp=0.0;fp<=(0.01e9)/f0;fp=fp+(1e4)/f0)
    { 
      double yp = 0.0;
      {
