...
int main(int argc, char *argv[]) {
  ...
  const char *mydirname = "MMTL-out";
    ...
    for (double z = -04.0; z <= -1.0; z=z+0.1)
    {
    	theta=k*(z+04.0);
    	...
    	zscoord[numcoord]=z+05.0;
    	numcoord++;
    }

	for(double y=-12.5;y<=12.5;y=y+12.5)
	{for (double z=-07.5;z<=07.5;z=z+00.1){
 ...
 for (double fp=0.0;fp<=3.0*(1e9/f0);fp=fp+0.5*(1e9/f0))
    { cout<<fp<<endl;
      for (double yp = -20.0; yp <= 20.0; yp=yp+10.0)
      {
    ...
    zcenp=zcen;
    volume box1( vec(dxmax,-dymax,-05.0), vec(-dxmax,dymax,0.0) );
    ycenp=ycen+dymin+(0.5*wcore); 
    volume box2( vec(dxmax,-dymax,0.0), vec(-dxmax,dymax,05.0) );
  ...
  */
  ...
  f.step();
  cout<<"Okay"<<endl;
    volume vxy=volume(vec(-xsize+10,-ysize+10,0),vec(xsize-10,ysize-10,0));
    volume vxz=volume(vec(-xsize+10,0,-zsize+10),vec(xsize-10,0,zsize-10));
    volume vyz=volume(vec(0,-ysize+10,-zsize+10),vec(0,ysize-10,zsize-10));
...
int stop=0;
    ...
    f.output_hdf5(Sz,vyz);
       ...
       if ((i%100)==0)
      {
        ...
        monitor_point pin;
        f.get_point(&pin, vec(xcen,ycen,-02.5));
        ...
        monitor_point po;
        f.get_point(&po, vec(xcen,ycen,02.5));
	...
	*/
    ...
    for (double y=-05.0;y<=05.0;y=y+00.01)  
    {
    	monitor_point pin;
    ...
    for (double z=-05.0;z<=05.0;z=z+00.1)  
    {
