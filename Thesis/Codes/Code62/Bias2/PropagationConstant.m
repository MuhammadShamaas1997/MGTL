...
for ind1=1:64
    for ind2=1:121
        E(ind1,ind2)=sqrt(Ex(ind1,ind2).*Ex(ind1,ind2)+Ey(ind1,ind2).*Ey(ind1,ind2)+Ez(ind1,ind2).*Ez(ind1,ind2));
        H(ind1,ind2)=sqrt(Hx(ind1,ind2).*Hx(ind1,ind2)+Hy(ind1,ind2).*Hy(ind1,ind2)+Hz(ind1,ind2).*Hz(ind1,ind2));
    end
end
...
E=[zeros(L2-64,121); E(1:64,:)];
H=[zeros(L2-64,121); H(1:64,:)];
...
hold on
for dist=1:121
plot3([0 real(Hx(131072,dist))*H0],[0 real(Hy(131072,dist))*H0], [dist dist+1]);
%axis([-1e-4 1e-4 0 122 -1e-4 1e-4])
end
...
hold on
for dist=1:121
plot([dist dist+0.1],[0 atan(abs(Hy(time1,dist))/abs(Hx(time1,dist)))*(180/pi)]);
end
...
FHxi=(fft((H(1:L,obs)),L));
FHxo=(fft((H(1:L,obs+dobs)),L));
...
FEx=fft((E(1:L,obs)),NFFT);
FHx=fft((H(1:L,obs)),NFFT);
Z=FEx./FHx;
Z=Z*377;
