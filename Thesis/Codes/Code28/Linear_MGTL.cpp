...
double dxmin=0.0, dxmax=1, dymin=0.0, dymax=7.5, dzmin=0.0, dzmax=1;
...
int main(int argc, char *argv[]) {
  ...
  const char *mydirname = "MMTL-out";
    ...
    double xsize=6, ysize=20, zsize=6;
  ...
  */
    ...
    meep_geom::material_type_list * mtl= new meep_geom::material_type_list();    
    mtl->num_items=1;
    mtl->items=new meep_geom::material_type;
    mtl->items[0]=my_material;
    ...
    set_materials_from_geometry(&transformer, g, center, use_anisotropic_averaging,DEFAULT_SUBPIXEL_TOL, DEFAULT_SUBPIXEL_MAXEVAL,ensure_periodicity, (my_material), al, *mtl,1.234);

    double maxf=1000;
    for (double f=-9; f<=9; f=f+0.1){
      //if(transformer.get_mu(vec(0.1,6.9,f),1)!=1000)
        {cout<<transformer.get_mu(vec(0.1,f,0),1)<<endl;}
    }
    ...
    */
      ...
      const volume vsrc3 =volume(vec(1.1,-7,-1.1), vec(-1.1,-6,-1.1));
      const volume vsrc4 =volume(vec(1.1,-7,1.1), vec(-1.1,-6,1.1));
      f.add_volume_source(Ex, src, vsrc3, cdouble(-amplitude,0));
